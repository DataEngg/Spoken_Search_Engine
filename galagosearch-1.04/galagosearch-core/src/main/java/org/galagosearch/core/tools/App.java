...
public class App {
	...
	private void handleDumpIndex(String[] args) throws IOException {
		...
		StructuredIndexPartReader reader = StructuredIndex
				.openIndexPart(args[1]);
	...
	private void handleSearch(Retrieval retrieval, DocumentStore store)
			throws Exception {
		...
		output.println("Server: http://" + 127 + "." + 0 + "." + 0 + "." + 1
				+ ":" + port);
		
	}

	private DocumentStore getDocumentStore(String[] corpusFiles)
			throws IOException {
		DocumentStore store = null;
		if (corpusFiles.length > 0) {
			//System.out.println("length "+corpusFiles.length);
	...
	private void handleSearch(String[] args) throws Exception {
		...
		//System.out.println("APP.java " + flags.toString());
	...
	public static Job getDocumentConverter(String outputCorpus, String[] inputs)
			throws IOException {
		Job job = new Job();
	...
	public void commandHelp(String command) throws IOException {
		if (command.equals("batch-search")) {
			...
			String[] commands = { "batch-search", "build", "doc",
					"dump-connection", "dump-corpus", "dump-index",
					"dump-keys", "eval", "make-corpus", "search" };
