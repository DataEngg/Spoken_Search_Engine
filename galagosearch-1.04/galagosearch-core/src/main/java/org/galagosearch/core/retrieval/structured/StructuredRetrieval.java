...
public class StructuredRetrieval extends Retrieval {
	...
	public StructuredRetrieval(StructuredIndex index,
		...
		featureParameters.add("collectionLength",
				Long.toString(index.getCollectionLength()));
		featureParameters.add("documentCount",
				Long.toString(index.getDocumentCount()));
	 ...
	 * @param requested
	 *            The number of documents to retrieve, at most.
	 * @return
	 * @throws java.lang.Exception
	 */
	public ScoredDocument[] runQuery(Node queryTree, int requested)
			throws Exception {
        ...
        if(SearchWebHandler.value1.equals("1"))
        {  
		while (!iterator.isDone()) {
			...
			if (queue.size() <= requested || queue.peek().score < score) {
				ScoredDocument scoredDocument = new ScoredDocument(document,
						score);
        ...
        {   //System.out.println("You have choosen "+SearchWebHandler.value1);
        	while (!iterator.isDone()) {
    			int document = iterator.nextCandidate();
    			int length = index.getLength(document);
    			//StructuredIndexPartReader rt=index.parts.get("stemmed");
    			//PositionIndexReader.Iterator it=(Iterator) iterator;
    			//System.out.println("Document in Structured Retrieval "+it.document());
    			//System.out.println("count of term in Structured Retrieval "+it.count());
    			//System.out.println("Term in Structured Retrieval "+it.getCurrentTerm());
    			
    			//double score = iterator.score(document, length);
    			double score1 = iterator.score1(document, length);
    			if (queue.size() <= requested || queue.peek().score < score1) {
    				ScoredDocument scoredDocument = new ScoredDocument(document,
    						score1);
    				queue.add(scoredDocument);
	...
	public ScoredDocument[] runQuery1(Node queryTree, int requested)
			throws Exception {
		// construct the query iterators
		ScoreIterator iterator = (ScoreIterator) createIterator(queryTree);
